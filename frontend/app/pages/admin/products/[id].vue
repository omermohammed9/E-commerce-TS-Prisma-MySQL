...
const { data: product, pending } = await useAsyncData(`admin-product-${route.params.id}`, () => 
...
watch(product, (newVal) => {
  if (newVal) {
    ...
    form.stockQuantity = newVal.stockQuantity
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Update Failed', description: error.data?.message, color: 'red' })
  } finally {
...
useHead({
  title: `Edit ${product.value?.name || 'Product'} | Admin`
})
</script>
...
<template>
  <UContainer class="py-12 max-w-3xl">
    <div class="mb-10 flex items-center gap-4">
      ...
      <div>
        <h1 class="text-4xl font-black text-gray-900 dark:text-white">Edit Product</h1>
        <p class="mt-1 text-gray-500 dark:text-gray-400">Update product details and stock information.</p>
      </div>
    </div>
    ...
    <UCard v-else class="p-8 rounded-3xl shadow-sm border-none bg-white dark:bg-gray-900">
